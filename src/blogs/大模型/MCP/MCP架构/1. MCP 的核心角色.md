...
// Client -> Server 发送的请求
{
  ...
  "params": {
    ...
    "capabilities": {          // Client 自身具备的能力声明
      "roots": { // 表示 Client 支持 Server 访问文件系统根目录
        "listChanged": true // 支持监控文件根目录的变化
       },
      "sampling": {}           // 表示支持 Server 回调 LLM
    },
    "clientInfo": {            // Client 的元数据信息
...
```json
{
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "region": { "type": "string", "description": "两位数国家代码，如 US, CN" }
    },
